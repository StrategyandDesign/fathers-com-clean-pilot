...
Why they existed at the repo root: the old pipeline generated pages with Python and committed the HTML. That work predates the React app. Duplicate `assets/`, `content/`, `data/`, and `tools/` copies were later removed from the Next.js root so the review repo is one product.
