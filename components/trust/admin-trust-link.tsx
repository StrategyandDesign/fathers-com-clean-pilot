...
export async function AdminTrustLink() {
  ...
  return (
    <details className="group rounded-xl border border-border/70 bg-card/60 open:[&_svg]:rotate-180">
      ...
      >
        <span className="min-w-0 text-sm text-muted-foreground">{t("trust.packSummary")}</span>
        <ChevronDown
          aria-hidden
      ...
      <div className="space-y-3 border-t border-border/70 px-4 py-4 sm:px-5">
        <p className="text-sm text-muted-foreground">{t("trust.packAccountLead")}</p>
        <Link
          href="/admin/trust"
