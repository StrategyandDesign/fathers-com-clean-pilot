...
export async function CounselAccountLink({
  href,
}: {
  href: "/manager/account/counsel" | "/admin/account/counsel";
}) {
  ...
  return (
    <details className="group rounded-xl border border-border/70 bg-card/60 open:[&_svg]:rotate-180">
      ...
      >
        <span className="min-w-0 text-sm text-muted-foreground">{t("counsel.summary")}</span>
        <ChevronDown
          aria-hidden
      ...
      <div className="space-y-3 border-t border-border/70 px-4 py-4 sm:px-5">
        <p className="text-sm text-muted-foreground">{t("counsel.accountLead")}</p>
        <Link
          href={href}
