...
- Review cadence on `/manager` keeps three buckets glanceable. Open items are men mid-work (a session still open, or no assigned training). Pending actions wait on the Leader (a catalog review or a certificate to send). Certificates ready are the certificate subset. The Leader roster shows film, checkpoint, and practice lights plus one next action. Nothing sends or issues until the Leader confirms. Father Home loads a new leader update on its own. Local `npm run dev` follows the latest Shared desk branch so a terminal checkout is not required.
