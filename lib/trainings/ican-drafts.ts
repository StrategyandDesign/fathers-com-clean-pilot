...
export function renderReturnHomeDraftDescriptionUpdateSql() {
  return renderReturnHomeFatherCopyUpdateSql({
    heading: `-- Update father-facing descriptions for the three return-home Super-admin drafts.
...
export function renderReturnHomeFatherCopyUpdateSql(options?: {
  heading?: string;
  ...
  const heading =
    options?.heading ??
    `-- Update father-facing descriptions for the three return-home trainings.
-- Does not change sessions, titles, published, released_at, or development_status.
-- Idempotent: re-run updates description and leader_summary in place by slug.
-- Leaves publish and release flags alone so an already-released Calm row stays usable.`;
  const publishedLine = options?.forceUnpublished
    ? ",\n  published = false"
    : "";
...
set
  ...
  leader_summary = catalog.leader_summary${publishedLine}
from (
  values
${values}
