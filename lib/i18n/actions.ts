...
import { parseLocalePreference } from "@/lib/i18n/org-locale";
import { allowedLocalesForUser, resolveUserLocale } from "@/lib/i18n/resolve";
...
export async function saveLocalePreference(formData: FormData) {
  ...
  const parsed = parseLocalePreference(raw, await allowedLocalesForUser(user.id));
  if ("error" in parsed) {
